...
{
    class PHandRightInFrontDetector: PostureDetector
    {
        private GlobalData.GestureTypes Name = GlobalData.GestureTypes.PHandRightInFront;

        public float Epsilon { get; set; }
        public float MaxRange { get; set; }

        public PHandRightInFrontDetector()
            : base(0)
        {
            Epsilon = 0.1f;
            MaxRange = 0.25f;
        ...
        private bool check(Vector3? spine, Vector3? handPosition)
        {
            ...
            if (spine.Value.Z < handPosition.Value.Z || spine.Value.Z - handPosition.Value.Z < 0.3)
                return false;
