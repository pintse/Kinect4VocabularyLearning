...
{
    class PHandRightOnRightDetector : PostureDetector
    {
        private GlobalData.GestureTypes Name = GlobalData.GestureTypes.PHandRightOnRight;

        public float Epsilon { get; set; }
        public float MaxRange { get; set; }

        public PHandRightOnRightDetector()
            : base(0)
        {
            Epsilon = 0.1f;
            MaxRange = 0.25f;
        ...
        private bool check(Vector3? spine, Vector3? handPosition)
        {
            ...
            if (handPosition.Value.X - spine.Value.X < 0.45)
                return false;
