...
{
    class PHandRightBelowDetector : PostureDetector
    {
        private GlobalData.GestureTypes Name = GlobalData.GestureTypes.PHandRightBelow;

        public float Epsilon { get; set; }
        public float MaxRange { get; set; }

        public PHandRightBelowDetector()
            : base(0)
        {
            Epsilon = 0.1f;
            MaxRange = 0.25f;
        ...
        private bool check(Vector3? handLeft, Vector3? handRight, Vector3? elbowLeft, Vector3? elbowRight)
        {
            ...
            if (handRight.Value.X > handLeft.Value.X || handLeft.Value.Y < handRight.Value.Y ||
                handLeft.Value.Y < elbowRight.Value.Y || (handLeft.Value.Y - handRight.Value.Y) < 0.1)
                return false;
