...
{
    class PHandRightInBackDetector : PostureDetector
    {
        private GlobalData.GestureTypes Name = GlobalData.GestureTypes.PHandRightInBack;

        public float Epsilon { get; set; }
        public float MaxRange { get; set; }

        public PHandRightInBackDetector()
            : base(0)
        {
            Epsilon = 0.1f;
            MaxRange = 0.25f;
        ...
        private bool check(Vector3? spine, Vector3? handPosition)
        {
            ...
            if (handPosition.Value.Z < spine.Value.Z || handPosition.Value.Z - spine.Value.Z < 0.3)
                return false;
